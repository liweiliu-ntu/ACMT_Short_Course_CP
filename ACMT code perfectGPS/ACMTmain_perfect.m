...
for ii=2:length(q11)
    %ggg=ii;
       ...
       if nextyf>0
        ...
        gg1=toc;
        gg=[gg,gg1];
             q1=q(ii-1,:)+dq*ts; Q=nextQ; dq=nextdq; dQ=nextdQ; qp=nextqp; qe=nextqe; 
            state1=[state1;(ii-2)*dt+ts dq ];
            state2=[state2; [(ii-2)*dt+ts Q dQ nextyf qp q1 iflag]];
   
            iflag=[1];
            tic;
            [nextQ,nextq,nextdq,nextdQ,nextyf,nextqp,nextdqp]=on_phase(q,ke,ts,Qy,Q,qp,dq,qy,ee,ii);
        gg1=toc;
        gg=[gg,gg1];
       else
           iflag=[0];
